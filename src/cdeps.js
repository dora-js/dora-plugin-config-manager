...
import { isExistedAFile } from './util';
...
function getFile(f) {
  ...
  if (endWith(f, '.js') && isExistedAFile(f)) return f;
  if (!endWith(f, '.js')) {
    if (isExistedAFile(`${f}.js`)) return `${f}.js`;
    if (isExistedAFile(`${f}/index.js`)) return `${f}/index.js`;
  }
